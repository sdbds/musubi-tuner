...
Qwen3 and whose teacher condition can be produced by a matching Qwen3-VL model.
...
- For Z-Image full-parameter fine-tuning, student and teacher use the same live
  DiT and the teacher weights are a CPU-backed EMA of all trainable DiT
  parameters.
...
| FLUX.2 Klein 4B | supported | Qwen3-4B student TE and Qwen3-VL-4B teacher. |
| FLUX.2 Klein 9B | supported | Qwen3-8B student TE and Qwen3-VL-8B teacher. |
...
The VLM weight source is provided by:
...
from the official Qwen3-VL repo by model variant:

```text
Z-Image:         Qwen/Qwen3-VL-4B-Instruct
FLUX.2 Klein 4B: Qwen/Qwen3-VL-4B-Instruct
FLUX.2 Klein 9B: Qwen/Qwen3-VL-8B-Instruct
```

The teacher VLM language-model reweight source is the same `--text_encoder`
used by the student path. There is intentionally no separate reweight-source
option. Supplying a different Qwen3 model would create a silent feature-space
mismatch risk.
...
The teacher cache script stacks the same Qwen3 hidden layers as the student
`Qwen3Embedder`:
...
- A Qwen3-VL path requires a transformers version older than `4.57.6`.

## Risk Register

- FLUX.2 Klein 9B depends on a matching Qwen3-VL-8B teacher and Qwen3-8B
  student text encoder.
- Full-parameter Z-Image D-OPSD keeps EMA and swap backups on CPU. It is much
  slower than adapter D-OPSD and consumes a full extra DiT-sized CPU/RAM copy.
