...
`--base_weights` merges one or more LoRAs into the BF16 base before training, and accepts the ComfyUI key format (`diffusion_model.blocks.N....lora_A/lora_B.weight`, alpha = rank) that the third-party de-distillation training adapters and ai-toolkit LoRAs use, in addition to Musubi's own format. A merged training adapter lifts the base out of the distilled guided space so that plain flow-matching training works; the trained LoRA is then used at inference on the plain base, without the adapter. Combining `--base_weights` with `--h3_guidance_loss_scale` or `--h3_teacher_matching` is allowed (a style LoRA under a character LoRA is an ordinary use) but logs a warning: the adapter authors advise against the guidance loss on top of it, and under teacher matching the merged base becomes the teacher. Training-time samples under a merged adapter show the de-distilled model and are not representative of the plain base + LoRA; evaluate with the generation script instead. A ConvRot INT8 base cannot be merged into, so `--base_weights` is rejected together with `--convrot_int8` and pre-quantized checkpoints for now.
