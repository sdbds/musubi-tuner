...
Every H3 timestep option (`--min/max_timestep`, the timestep focus below, `--h3_guidance_loss_sigma_min`, `--h3_teacher_condition_sigma_min/max`) is specified in this base space. The per-stream sigmas under the default shifts are:
...
`--base_weights` merges one or more LoRAs into the BF16 base before training, and accepts the Diffusers key format (`diffusion_model.blocks.N....lora_A/lora_B.weight`, alpha = rank) that the third-party de-distillation training adapters and ai-toolkit LoRAs use, in addition to Musubi's own format. A LoRA loaded from weights is applied to every module it contains, including token refiner modules that Musubi's own training default leaves out. A merged training adapter lifts the base out of the distilled guided space so that plain flow-matching training works; the trained LoRA is then used at inference on the plain base, without the adapter. Combining `--base_weights` with `--h3_guidance_loss_scale` or `--h3_teacher_matching` is allowed (a style LoRA under a character LoRA is an ordinary use) but logs a warning: the adapter authors advise against the guidance loss on top of it, and under teacher matching the merged base becomes the teacher. Training-time samples under a merged adapter show the de-distilled model and are not representative of the plain base + LoRA; evaluate with the generation script instead. With `--convrot_int8` on a BF16 checkpoint, the `--base_weights` LoRAs are merged into the BF16 weights during the streaming load and quantized with them, so an adapter and the INT8 base combine at no extra memory cost. Pre-quantized INT8 checkpoints cannot be merged into and are rejected with `--base_weights`.
