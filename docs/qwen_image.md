...
- `--disable_numpy_memmap`: モデル読み込み時のnumpyメモリマッピングを無効化し、モデル全体をRAMに読み込んで読み込みを高速化します。RAM使用量は増加しますが、モデルの読み込みが大幅に高速化され、ブロックスワップやLoRA学習で特に有用です。
...
**注意:** `--disable_numpy_memmap`オプションは、numpyメモリマッピングの代わりにモデル全体をRAMに読み込むことで、モデルの読み込みを高速化します。`--blocks_to_swap`を使用する場合に特に有効で、学習中のモデルウェイトの読み込み時間を短縮します。
