    ...
    if tp_mesh.size() > 1:
        plan_in = {
            "out_layer": PrepareModuleInput(
                input_layouts=(Replicate(), None, None), desired_input_layouts=(Shard(1), None, None), use_local_output=True
            ),
        }
        ...
        for i, block in enumerate(model.visual_transformer_blocks):
            plan = {
                "self_attention_norm": SequenceParallel(sequence_dim=0, use_local_output=True),
                "self_attention.to_query": PrepareModuleOutput(output_layouts=(Shard(0)), desired_output_layouts=(Shard(-1))),
                "self_attention.to_key": PrepareModuleOutput(output_layouts=(Shard(0)), desired_output_layouts=(Shard(-1))),
                "self_attention.to_value": PrepareModuleOutput(output_layouts=(Shard(0)), desired_output_layouts=(Shard(-1))),
                ...
                "cross_attention_norm": SequenceParallel(sequence_dim=0, use_local_output=True),
                "cross_attention.to_query": PrepareModuleOutput(output_layouts=(Shard(0)), desired_output_layouts=(Shard(-1))),
                "cross_attention.to_key": PrepareModuleOutput(output_layouts=(Replicate()), desired_output_layouts=(Shard(-1))),
                "cross_attention.to_value": PrepareModuleOutput(output_layouts=(Replicate()), desired_output_layouts=(Shard(-1))),
            ...
            if i == len(model.visual_transformer_blocks) - 1:
                parallelize_module(
                    block,
                    tp_mesh,
                    PrepareModuleOutput(output_layouts=(Shard(0)), desired_output_layouts=(Replicate())),
                )
