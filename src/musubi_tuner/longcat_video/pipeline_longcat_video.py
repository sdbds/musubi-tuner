...
from musubi_tuner.longcat_video.utils.bucket_config import get_bucket_config
...
class LongCatVideoPipeline:
    ...
    def check_inputs(self, prompt, negative_prompt, height, width, scale_factor_spatial):
    ...
    def _cache_clean_latents(self, cond_latents, model_max_length, offload_kv_cache, device, dtype):
        ...
        empty_embeds = torch.zeros(
            [cond_latents.shape[0], 1, model_max_length, self.text_encoder.config.d_model], device=device, dtype=dtype
        )
    ...
    def generate_t2v(
        ...
        if prompt_embeds is None:
            ...
            if prompt_attention_mask is None:
                prompt_attention_mask = torch.ones((batch_size, prompt_embeds.shape[2]), dtype=torch.long, device=device)
            else:
            ...
            if self.do_classifier_free_guidance:
                if negative_prompt_embeds is None:
                    ...
                    if negative_prompt_embeds.dim() == 2:
                        ...
                        raise ValueError("negative_prompt_embeds must have shape [T, C], [B, T, C], or [B, 1, T, C]")
    ...
    def generate_vc(
        ...
        if enhance_hf:
            ...
            timesteps_uniform_tail = list(
                np.linspace(
                    tail_uniform_start, tail_uniform_end, num_tail_uniform_steps, dtype=np.float32, endpoint=(tail_uniform_end != 0)
                )
            )
        ...
        if use_kv_cache:
            ...
            self._cache_clean_latents(
                cond_latents, max_sequence_length, offload_kv_cache=offload_kv_cache, device=self.device, dtype=dit_dtype
            )
    ...
    def generate_refine(
        ...
        video_UP = F.interpolate(
            video_DOWN, size=(new_frame_size, height, width), mode="trilinear", align_corners=True
        )  # [B, C, frame, H, W]
        ...
        num_noise_latents = math.ceil((new_frame_size - num_cond_frames) / self.vae_scale_factor_temporal)
