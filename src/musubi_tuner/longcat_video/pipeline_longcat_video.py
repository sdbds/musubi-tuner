...
from musubi_tuner.longcat_video.modules.autoencoder_kl_wan import AutoencoderKLWan
from musubi_tuner.longcat_video.modules.longcat_video_dit import LongCatVideoTransformer3DModel
from musubi_tuner.longcat_video.context_parallel import context_parallel_util
from musubi_tuner.longcat_video.utils.bukcet_config import get_bucket_config
...
class LongCatVideoPipeline:
    ...
    def __init__(
        ...
        self.vae_scale_factor_temporal, self.vae_scale_factor_spatial = self._resolve_vae_scale_factors()
    ...
    def _resolve_vae_scale_factors(self) -> tuple[int, int]:
        config = getattr(self.vae, "config", None)

        if config is not None:
            return getattr(config, "scale_factor_temporal", 4), getattr(config, "scale_factor_spatial", 8)

        # WanVAE exposes scale information through its architecture; fall back to known defaults.
        # Temporal downsampling occurs twice by default (stride-2 Conv3d), spatial downsampling thrice (stride-2 Conv2d).
        # Adjust here if WanVAE variants change.
        temporal = getattr(self.vae, "temporal_downsample_factor", None)
        spatial = getattr(self.vae, "spatial_downsample_factor", None)
    ...
    def generate_t2v(
        ...
        if prompt_embeds is None:
            ...
            dummy_prompt = [""] * (
                prompt_embeds.shape[0] if prompt_embeds.dim() > 0 else 1
            )
            self.check_inputs(
                dummy_prompt,
                negative_prompt if negative_prompt_embeds is None else [""] * len(dummy_prompt),
                height,
                width,
                scale_factor_spatial,
            )
        ...
        if context_parallel_util.get_cp_rank() == 0:
            if prompt_embeds is None:
                ...
                if prompt_embeds.dim() == 2:
                    prompt_embeds = prompt_embeds.unsqueeze(0).unsqueeze(1)
                elif prompt_embeds.dim() == 3:
                    prompt_embeds = prompt_embeds.unsqueeze(1)
                elif prompt_embeds.dim() != 4:
                    raise ValueError("prompt_embeds must have shape [T, C], [B, T, C], or [B, 1, T, C]")
                if prompt_attention_mask is None:
                    prompt_attention_mask = torch.ones(
                        (batch_size, prompt_embeds.shape[2]), dtype=torch.long, device=device
                    )
                else:
                ...
                if self.do_classifier_free_guidance:
                    if negative_prompt_embeds is None:
                        ...
                        negative_prompt_attention_mask = prompt_attention_mask.clone()
                    else:
                        negative_prompt_embeds = negative_prompt_embeds.to(device=device, dtype=dit_dtype)
                        if negative_prompt_embeds.dim() == 2:
                            negative_prompt_embeds = negative_prompt_embeds.unsqueeze(0).unsqueeze(1)
                        elif negative_prompt_embeds.dim() == 3:
                        ...
                        if negative_prompt_attention_mask is None:
                            negative_prompt_attention_mask = torch.ones(
                                (batch_size, negative_prompt_embeds.shape[2]),
                                dtype=torch.long,
                                device=device,
                            )
                        else:
            ...
            if prompt_embeds is not None:
                if prompt_embeds.dim() == 4:
                    seq_len = prompt_embeds.shape[2]
                elif prompt_embeds.dim() == 3:
                    seq_len = prompt_embeds.shape[1]
                elif prompt_embeds.dim() == 2:
                    seq_len = prompt_embeds.shape[0]
                else:
                    raise ValueError("prompt_embeds must have shape [T, C], [B, T, C] or [B, 1, T, C]")
            else:
                seq_len = max_sequence_length
            prompt_embeds = torch.zeros([batch_size, 1, seq_len, caption_channels], dtype=dit_dtype, device=device)
            prompt_attention_mask = torch.zeros([batch_size, seq_len], dtype=torch.int64, device=device)
            context_parallel_util.cp_broadcast(prompt_embeds)
            context_parallel_util.cp_broadcast(prompt_attention_mask)
            if self.do_classifier_free_guidance:
                negative_prompt_embeds = torch.zeros([batch_size, 1, seq_len, caption_channels], dtype=dit_dtype, device=device)
                negative_prompt_attention_mask = torch.zeros([batch_size, seq_len], dtype=torch.int64, device=device)
                context_parallel_util.cp_broadcast(negative_prompt_embeds)
                context_parallel_util.cp_broadcast(negative_prompt_attention_mask)
    ...
    def generate_i2v(
        ...
        if prompt_embeds is None:
            ...
            dummy_prompt = [""] * (prompt_embeds.shape[0] if prompt_embeds.dim() > 0 else 1)
            self.check_inputs(
                dummy_prompt,
                negative_prompt if negative_prompt_embeds is None else [""] * len(dummy_prompt),
                height,
                ...
                if self.do_classifier_free_guidance:
                    if negative_prompt_embeds is None:
                        ...
                        if negative_prompt_embeds.dim() == 2:
                            ...
                            negative_prompt_embeds = negative_prompt_embeds.unsqueeze(1)
                        elif negative_prompt_embeds.dim() != 4:
                            raise ValueError(
                                "negative_prompt_embeds must have shape [T, C], [B, T, C], or [B, 1, T, C]"
                            )
                        if negative_prompt_attention_mask is None:
                            negative_prompt_attention_mask = torch.ones(
                                (batch_size, negative_prompt_embeds.shape[2]),
