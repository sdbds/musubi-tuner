...
class MiniMaxH3NetworkTrainer(NetworkTrainer):
    ...
    def _prepare_training_state(
        ...
        noisy_video, timesteps = self.get_noisy_model_input_and_timesteps(
            args, video_noise, latents, pool, noise_scheduler, device, dit_dtype
        )
    ...
    def process_batch(
        ...
        state, inputs = self._prepare_training_state(args, batch, latents, noise, noise_scheduler, dit_dtype)
    ...
    def process_batch_best_of_k(
        ...
        del sample_resources
        state, initial_inputs = self._prepare_training_state(args, batch, latents, noise, noise_scheduler, dit_dtype)
    ...
    def compute_loss(
        ...
        if not teacher_matching:
            ...
            return total.mean(), _scalar_logs(logs)
